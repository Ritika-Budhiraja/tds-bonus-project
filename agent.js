...
class jarvis {
    ...
    async init() {
        try {
            ...
            console.log(`jarvis v${this.version} initialized successfully`);
        } catch (error) {
    ...
    getDefaultSettings() {
        return {
            llm: { provider: 'google', apiKey: 'AIzaSyCwmCkF_mSoXKm3R7Kx-LfaWy4Spb7BfW4', model: 'gemini-1.5-flash', maxTokens: 2000, temperature: 0.7 },
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'jarvis', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to jarvis! How can I assist you?');
        }
    }
